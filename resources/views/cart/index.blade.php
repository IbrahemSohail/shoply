...
<section class="bg-white py-6 sm:py-8 antialiased md:py-12">
    <div class="mx-auto w-full px-4 sm:px-6 lg:max-w-screen-xl lg:px-8 2xl:px-0">
      <h2 class="text-lg sm:text-xl font-semibold text-gray-900">{{ __('Shopping Cart') }}</h2>
  
      <div class="mt-6 sm:mt-8 flex flex-col lg:flex-row gap-6 lg:gap-8">
        <div class="w-full lg:max-w-3xl flex-1">
           @if($cartItems->isEmpty())
        <div class="alert alert-info text-center py-4 px-4 bg-blue-50 border border-blue-200 rounded-lg text-sm sm:text-base">{{ __('Cart Empty') }}</div>
    @else
                ...
                <div class="flex flex-col items-end gap-4">
                  <div>
                    @if($item->product->offer_price)
                        <p class="text-sm text-gray-500 line-through">${{ number_format($item->product->price, 2) }}</p>
                        <p class="text-base sm:text-lg font-bold text-red-600">${{ number_format($item->product->offer_price, 2) }}</p>
                    @else
        ...
        <div class="w-full lg:w-96 space-y-6 lg:space-y-4">
          <div class="space-y-4 rounded-lg border border-gray-200 bg-white p-4 sm:p-6 shadow-sm">
            <p class="text-lg sm:text-xl font-semibold text-gray-900">{{ __('Order summary') }}</p>
  
            <div class="space-y-4 text-sm sm:text-base">
              <div class="space-y-2">
                <dl class="flex items-center justify-between gap-4">
                  <dt class="font-normal text-gray-500">{{ __('Original price') }}</dt>
                  <dd class="font-medium text-gray-900">${{ number_format($originalPrice, 2) }}</dd>
                </dl>
  
                <dl class="flex items-center justify-between gap-4">
                  <dt class="font-normal text-gray-500">{{ __('Savings') }}</dt>
                  <dd class="font-medium text-green-600">-${{ number_format($savings, 2) }}</dd>
                </dl>
  
                <dl class="flex items-center justify-between gap-4">
                  <dt class="font-normal text-gray-500">{{ __('Store Pickup') }}</dt>
                  <dd class="font-medium text-gray-900">${{ number_format($storePickup, 2) }}</dd>
                </dl>
  
                <dl class="flex items-center justify-between gap-4">
                  <dt class="font-normal text-gray-500">{{ __('Tax') }}</dt>
            ...
            <a href="{{route('checkout')}}" class="flex w-full items-center justify-center rounded-lg bg-gray-700 px-4 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm font-medium text-white hover:bg-gray-800 focus:outline-none focus:ring-4 focus:ring-gray-300 transition">{{ __('Checkout') }}</a>
